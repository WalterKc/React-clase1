...
export class Game extends React.Component {
  ...
  ganador(cuadros) {
    ...
    //los cuadros, los recibo del slice actual, asi que , hay que importalos
    ...
    for (let x = 0; x < 8; x++) {
      ...
      if (
        ...
        console.log(cuadros);
        //console.log("Gano? " + this.state.historial.Gano);
  ...
  LogicaDelClick(i) {
    ...
    const squares = actual.cuadros.slice();
    //revisa si hay algo
    console.log(squares[i]);
  ...
  render() {
    ...
    const actual = historial[this.state.contador];
    //const ganador = "";
    /*test map
    var numbers = [1, 2, 3];
    ...
    */
    ...
    const movimientos = historial.map((estadoActual, indiceMovActual) => {
      ...
      */
      ...
      let mensaje;
      //aca toma como valor, si tiene algo en el indice , cualquier cosa diferente de null y false
      ...
      return (
        <li key={indiceMovActual}>
          <button onClick={() => this.saltarA(indiceMovActual)}>
            {mensaje}
          </button>
        </li>
      );
